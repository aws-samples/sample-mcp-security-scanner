...
2. Run ALL applicable directory scanners **sequentially** (not in parallel) with `return_output=True` to avoid context window overflow. Always pass the absolute path — never `.` or a relative path (the MCP server cannot resolve relative paths reliably). Use `pwd` to get the absolute path first if needed:
